    ...
    override func presentationTransitionWillBegin() {
        ...
        self.presentationWrappingView = {
            ...
            let presentedViewControllerWrapperView: UIView = {
                let inset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
                let presentedViewControllerWrapperView = UIView(frame: presentationRoundedCornerView.bounds.inset(by: inset))
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        ...
        if presentedViewContentSize.height >= UIScreen.main.bounds.height - topinset - bottominset {
            ...
            let originOffset: CGPoint = {
                return CGPoint(
                    x: (UIScreen.main.bounds.width - presentedViewContentSize.width) / 2,
                    y: max(20, (UIScreen.main.bounds.height - presentedViewContentSize.height) / 2)
                )
            }()
            
            presentedViewControllerFrame.size = presentedViewContentSize
            presentedViewControllerFrame.origin = originOffset
        }
...
extension ModalPresentationController: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if (isPresenting) {
            ...
            toViewInitialFrame.origin = CGPoint(x: containerView.bounds.minX + 20, y: containerView.bounds.maxY)
